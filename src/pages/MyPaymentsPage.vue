<template>
  <div class="h-full overflow-y-scroll p-0 sm:p-6 bg-no-repeat bg-left-bottom"
    style="background-image: url('images/bottm-left-circles.svg');">

    <div class="max-w-7xl mx-auto pt-[100px] sm:pt-0 sm:p-8">
      <!-- Header -->
      <div class="w-full flex items-center justify-between mb-8 p-2">
        <h1 class="w-1/3 text-3xl font-bold text-gray-900 hidden sm:block">My payment</h1>
        <div class="w-full sm:w-2/3 grid grid-cols-2 sm:grid-cols-3 gap-2">
          <!-- Balance Card -->
          <div
            class="col-span-1 sm:col-span-2 bg-g text-white px-8 h-12 rounded-full flex items-center justify-between  shadow-lg">
            <span class="text-xs sm:text-sm font-medium">My Balance</span>
            <span class="text-sm sm:text-3xl font-bold">$453.00</span>
          </div>

          <!-- Request Withdrawal Button -->
          <button
            class="col-span-1 bg-purple-200 w-full text-purple-600 px-8 h-12 rounded-full text-xs sm:text-sm font-bold shadow-lg hover:shadow-xl transition-all duration-200">
            Request Withdrawal
          </button>
        </div>
      </div>

      <!-- Recent Transactions Section -->
      <div class="bg-glass overflow-hidden">
        <div class="px-2 sm:px-4 py-6">
          <h2 class="text-xl font-bold text-gray-800">Recent Transactions</h2>
        </div>

        <!-- Grid Container -->
        <pagination-comp>
          <div class="grid gap-4 w-full">
            <!-- Transaction Row -->
            <template v-for="(item, index) in 10" :key="index">
              <div v-for="(item, index) in dataList?.data" :key="index"
                class="flex items-center justify-between border rounded-md border-gray-100 py-4 px-0 px-2 sm:px-6 ">

                <!-- User -->
                <div class="row-span-1 flex items-center gap-4 min-w-0">
                  <div class="w-14 h-14 rounded-full overflow-hidden">
                    <img :src="showImg(item.userId?.profileImage)" :alt="item.userId?.name"
                      class="w-full h-full object-cover">
                  </div>
                  <div>
                    <p class="font-semibold text-gray-900 text-lg truncate">{{ item.userId?.name }}</p>
                    <p class="text-xs block sm:hidden">
                      <span class="text-[10px] text-gray-500">B. ID </span>
                      <booking-id :value="item.jobId"></booking-id>
                    </p>
                  </div>
                </div>

                <!-- Booking ID -->
                <div class="hidden sm:block row-span-1 text-sm text-right">
                  <span class="text-gray-500 ">B. ID </span>
                  <booking-id :value="item.jobId" class="text-gray-800 font-bold"></booking-id>
                </div>

                <!-- Date -->
                <div class="row-span-1 text-sm text-gray-500 text-right flex flex-col justify-center items-center">
                  {{ getDate(item.createdAt) }}
                  <br>
                  <span class="block sm:hidden">{{ getTime(item.createdAt) }}</span>
                </div>

                <!-- Time -->
                <div class="hidden sm:block row-span-1 text-sm text-gray-500 text-right">
                  {{ getTime(item.createdAt) }}
                </div>

                <!-- Amount -->
                <div class="row-span-1 text-right">
                  ${{ item.amount?.toFixed(2) }}
                </div>
                <div class="row-span-1 text-sm text-gray-500 text-right flex flex-col justify-center items-center">
                  <span class="font-bold text-purple-500 text-lg">${{ item.amount?.toFixed(2) }}</span>
                  
                  <span class="block sm:hidden"><br>{{ getTime(item.createdAt) }}</span>
                </div>
              </div>
            </template>
          </div>
        </pagination-comp>
      </div>
    </div>

    <div class="hidden h-[86px] w-full fixed top-0 left-0 bg-glass shadow-lg rounded-b-lg"></div>
  </div>
</template>

<script>
import BookingId from '@/components/BookingId.vue';
import PaginationComp from '@/components/PaginationComp.vue';

export default {
  name: "MyPaymentsPage",
  components: { PaginationComp, BookingId },
  data() {
    return {
      //
    };
  },
  mounted() {
    this.fetchData();
  },
};
</script>